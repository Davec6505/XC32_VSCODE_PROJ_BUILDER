...
if "%choice%"=="1" (
    ...
    bash "%~dp0..\cmd\generate_project.cmd" . ExampleBasic
) else if "%choice%"=="2" (
    echo Running: generate_project.cmd ExampleDevice 32MZ2048EFH064
    bash "%~dp0..\cmd\generate_project.cmd" ExampleDevice . 32MZ2048EFH064
) else if "%choice%"=="3" (
    :: Running: python generate_project.py custom name custom directory
    :project_name_prompt
    set /p PROJECT_NAME="Enter project name: "
    ...
    if not exist "!CUSTOM_DIR!" mkdir "!CUSTOM_DIR!"
    echo Running:  generate_project.cmd !PROJECT_NAME! -o !CUSTOM_DIR!
    cd /d "%~dp0..\cmd" && generate_project.cmd !PROJECT_NAME! -o !CUSTOM_DIR!

) else if "%choice%"=="4" (
    echo Running: generate_project.cmd ExampleMikroC 32MZ1024EFH064 C:\Temp mikroc
    bash "%~dp0..\cmd\generate_project.cmd" ExampleMikroC 32MZ1024EFH064 C:\Temp mikroc
) else if "%choice%"=="5" (
   goto :exiting
) else (
    echo Invalid choice. Please run the script again.
