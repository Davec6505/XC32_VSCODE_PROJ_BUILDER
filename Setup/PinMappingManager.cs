    ...
    public class PinMappingManager
    {
        ...
        public void LoadPinConfigurationsFromConfig(DeviceConfig config)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Loading pin configurations: {pinConfigSection.Count} entries found");

                // Try to load from new individual pin format first
                bool foundIndividualPins = false;
                foreach (var kvp in pinConfigSection)
                {
                    try
                    {
                        if (kvp.Key.StartsWith("Pin") && kvp.Key.Length > 3)
                        {
                            // Extract pin number from "Pin1", "Pin2", etc.
                            string pinNumberStr = kvp.Key.Substring(3);
                            
                            if (int.TryParse(pinNumberStr, out _))
                            {
                                foundIndividualPins = true;
                                dynamic pinConfig = JsonConvert.DeserializeObject(kvp.Value);

                                if (_textBoxControls.TryGetValue(pinNumberStr, out var textBox) && textBox != null)
                                {
                                    textBox.Text = pinConfig.PinName?.ToString() ?? "";
                                    System.Diagnostics.Debug.WriteLine($"Loaded pin {pinNumberStr} name: {textBox.Text}");
                                }

                                if (_comboBoxControls.TryGetValue(pinNumberStr, out var comboBox) && comboBox != null)
                                {
                                    string selectedFunction = pinConfig.SelectedFunction?.ToString() ?? "GPIO";
                                    
                                    // First check if the ComboBox has items populated
                                    if (comboBox.Items.Count == 0)
                                    {
                                        System.Diagnostics.Debug.WriteLine($"ComboBox for pin {pinNumberStr} has no items, skipping function selection");
                                        continue;
                                    }
                                    ...
                                    if (comboBox.Items.Contains(selectedFunction))
                                    {
                                        ...
                                        System.Diagnostics.Debug.WriteLine($"Loaded pin {pinNumberStr} function: {selectedFunction}");
                                    }
                                    else
                                    {
                                        System.Diagnostics.Debug.WriteLine($"Function {selectedFunction} not available for pin {pinNumberStr} (available: {string.Join(", ", comboBox.Items.Cast<object>())}), keeping current selection");
                                    }
                                }
                            }
                        }
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"Error loading individual pin {kvp.Key}: {ex.Message}");
                    }
                }

                // Fall back to old format if no individual pins found
                if (!foundIndividualPins)
                {
                    // Try to load from old single "Data" entry format
                    if (pinConfigSection.TryGetValue("Data", out var jsonData))
                    {
                        System.Diagnostics.Debug.WriteLine("Loading from old format (single Data entry)");
                        ...
                        foreach (var kvp in pinConfigurations)
                        {
                            ...
                            {
                                System.Diagnostics.Debug.WriteLine($"Error loading pin {kvp.Key} from old format: {ex.Message}");
                            }
                        }
                    }
                    else
                    {
                        // Legacy format - direct pin number keys
                        LoadFromOldFormat(pinConfigSection);
                    }
                }

                System.Diagnostics.Debug.WriteLine("Pin configuration loading completed");
            }
            catch (Exception ex)
            {
                    ...
                    if (int.TryParse(kvp.Key, out _)) // It's a pin number
                    {
                        try
                        {
                            ...
                            if (_comboBoxControls.TryGetValue(pinNumber, out var comboBox) && comboBox != null)
                            {
                                ...
                                if (comboBox.Items.Count == 0)
                                {
                                    System.Diagnostics.Debug.WriteLine($"ComboBox for pin {pinNumber} has no items (old format), skipping function selection");
                                    continue;
                                }
